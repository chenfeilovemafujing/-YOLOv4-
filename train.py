...
def train(config):
    ...
    if use_gpu:
        criterion = loss_function.MS_SSIM(data_range=1, size_average=True, channel=3).cuda()
    else:
        criterion = loss_function.MS_SSIM(data_range=1, size_average=True, channel=3)
    ...
    for epoch in range(config.num_epochs):
        for iteration, (img_orig, img_haze) in enumerate(train_loader):
            ...
            if not use_gpu:
                print("Epoch[%d/%d], Loss at iteration %d/%d" % (epoch + 1, config.num_epochs, iteration + 1, train_loader.__len__()), ":", loss.item())
            elif ((iteration+1) % config.display_iter) == 0:
                print("Epoch[%d/%d], Loss at iteration %d/%d" % (epoch + 1, config.num_epochs, iteration + 1, train_loader.__len__()), ":", loss.item())
        ...
        torch.save(dehaze_net.state_dict(), config.snapshots_folder + "Epoch_" + str(epoch + 1) + '_dehazer.pth')
